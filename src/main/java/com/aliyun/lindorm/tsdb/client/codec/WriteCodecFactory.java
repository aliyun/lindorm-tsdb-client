...
public class WriteCodecFactory {
    ...
    public static byte[] encode(List<Record> records, CodecType type) {
        switch (type) {
            case JSON:
                return JSON_CODEC.encode(records);
            case BINARY:
                return BINARY_CODEC.encode(records);
        }
        throw new UnsupportedOperationException("Unsupported serialize type: " + type.name());
    }
}
